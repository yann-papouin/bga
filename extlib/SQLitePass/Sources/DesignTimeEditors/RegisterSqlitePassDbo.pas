...
{$ENDIF}
 ...
 SqlitePassCreateNewDatabaseDialog, SqlitePassParamsDialog, SqlitePassErrorLogDialog,
...
TDatasetParamsEditor = class(TStringProperty)
  Public
    Procedure Edit;override;
    Function  GetAttributes: TPropertyAttributes; override;
  End;
...
begin
 ...
 RegisterPropertyEditor(TypeInfo(TSqlitePassParams),TSqlitePassDataset,'Params',TDatasetParamsEditor);
...
if Assigned(Dataset.Database) then
   ...
   if CheckDatabaseconnected(Dataset.Database, Msg1027) then
      begin
      F := TSqlitePassChooseDatasetDlg.Create(Dataset.Database);
...
if Dataset.Active then
   begin
   F := TSqlitePassIndexedByDialog.Create(Dataset);
   if (F.ShowModal = mrOk)
...
begin
Dataset:=TSqlitePassDataset(GetComponent(0));
F := TSqlitePassMasterDetailDialog.Create(Dataset);
F.ShowModal;
F.Free;
end;
...
{ TDatasetParamsEditor }

procedure TDatasetParamsEditor.Edit;
var
Dataset: TSqlitePassDataset;
F: TSqlitePassParamsDlg;
begin
Dataset:=TSqlitePassDataset(GetComponent(0));
F := TSqlitePassParamsDlg.Create(Dataset);
F.ShowModal;
F.Free;
end;

function TDatasetParamsEditor.GetAttributes: TPropertyAttributes;
begin
  Result:= [paDialog];
end;
...
Case Index of
    ...
    8: begin
       E := TSqlitePassErrorLogDlg.Create(Database.DatabaseError);
       E.ShowModal;
       E.Free;
       end;
    end;
